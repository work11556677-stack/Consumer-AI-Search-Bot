...
def create_system_prompt(use_case, last_query_reference=None):
    if use_case != "use_case_3":
        ...
        base_persona = (
            ...
            f"You must expand the report with specific relation to the given question, and the dotpoint that the user wants to focus on:  {last_query_reference}"
...
def main_answer(query, candidates_block, sources_text, use_case, last_query_reference=None):
    # define persona, based on use_case
    persona, rules = create_system_prompt(use_case, last_query_reference=last_query_reference)
